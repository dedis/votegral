...
func NewFlow(o *actors.ElectionOfficial, k *actors.RegistrationKiosk, ea *actors.ElectionAuthority, p *actors.EnvelopePrinter, l *ledger.Ledger, hw hardware.Hardware) *Flow {
...
func (f *Flow) CheckIn(ctx *context.OperationContext, voter *actors.Voter) (*io.CheckInBarcode, error) {
	var barcode *io.CheckInBarcode

	// Use a temporary storage for the barcode state
	barcodeTempStorage := io.NewSimpleStorage()

	err := ctx.Recorder.Record("Official_CheckIn", func() error {
	...
	err = ctx.Recorder.Record("Kiosk_Authorization", func() error {
		// Read simulates scanning the code from the physical ticket.
...
func (f *Flow) CreateRealCredential(ctx *context.OperationContext, voter *actors.Voter, barcode *io.CheckInBarcode) (*io.VotingMaterials, error) {
	var materials *io.VotingMaterials
	err := ctx.Recorder.Record("Real_Credential_Creation", func() error {
		var err error
		materials, err = f.kiosk.GenerateCredentialMaterials(ctx, voter, f.ea, f.printer, f.hw, false, nil)
		return err
	})
...
// We need the checkout ticket from the Real voting materials to derive the necessary
// information to create the test credential.
func (f *Flow) CreateTestCredential(ctx *context.OperationContext, voter *actors.Voter) (*io.VotingMaterials, error) {
	var materials *io.VotingMaterials

	if voter.RealMaterial() == nil || voter.RealMaterial().CheckOut == nil {
		return nil, fmt.Errorf("cannot create test credential without a valid real checkout ticket")
	}
	realCheckoutTicket := voter.RealMaterial().CheckOut

	err := ctx.Recorder.Record("Test_Credential_Creation", func() error {
		var err error
		// The test credential creation needs the checkout ticket to derive the ElGamal secret.
		materials, err = f.kiosk.GenerateCredentialMaterials(ctx, voter, f.ea, f.printer, f.hw, true, realCheckoutTicket)
		return err
	})
	return materials, err
}
...
func (f *Flow) CheckOut(ctx *context.OperationContext, voter *actors.Voter, material *io.VotingMaterials) error {
	return ctx.Recorder.Record("CheckOut", func() error {
...
// It checks all signatures, verifies the ZKP, and validates against the public ledgers.
func (f *Flow) Activate(ctx *context.OperationContext, voter *actors.Voter, m *io.VotingMaterials) error {
	return ctx.Recorder.Record("Activation", func() error {
		// 1. Simulate reading all materials from the provided configuration.
...
// CreateVote creates an encrypted vote containing their credential and vote.
// This function will produce the `ledger.VotingEntry` needed for tallying.
func (f *Flow) CreateVote(ctx *context.OperationContext, materials *io.VotingMaterials, vote int) (*ledger.VotingEntry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create proof for credential encryption: %w", err)
	}

	// Encrypt Vote with Proof
	encVote, encVoteProof, err := CreateVoteWithProof(f.ea.PublicKey(), vote)
	if err != nil {
		return nil, fmt.Errorf("failed to create vote with proof: %w", err)
	}
	...
	if err = votingEntry.Sign(materials.Credential.PrivateKey()); err != nil {
		return nil, fmt.Errorf("failed to sign voting entry: %w", err)
	}
	return votingEntry, err
}
